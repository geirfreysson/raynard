...
describe('main agent core', () => {
  ...
  it('keeps Telegram turns useful while excluding desktop confirmation flows', () => {
    const prompt = buildMainAgentSystemPrompt({
      mode: 'explore',
      executionSurface: 'telegram',
      toolNames: ['oecd_data'],
      memories: [{ id: 'mem-1', scope: 'global', content: 'Prefers concise answers.' }]
    });
    ...
    expect(prompt).not.toContain('propose_memory_change');
  });
